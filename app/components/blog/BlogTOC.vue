...
function sync() {
  ...
  requestAnimationFrame(() => {
    ...
    if (visibleItems > 0 && pathStart < pathEnd) {
      if (
        pathStart !== lastPathStart.value
        || pathEnd !== lastPathEnd.value
      ) {
...
<template>
  ...
  >
    <nav
      ref="navRef"
      class="scrollbar-hide max-h-[80vh] w-72 overflow-auto overscroll-contain"
    >
      <div class="flex items-center gap-3 px-2 text-muted-foreground">
      ...
      <ul ref="tocRef" class="toc p-2 pr-4 text-sm">
        <li v-for="item in flatToc" :key="item.id">
          <a
            :href="`#${item.id}`"
            :class="cn(
              'flex h-7 items-center underline underline-offset-2 decoration-muted/40 hover:text-foreground hover:decoration-muted transition-all duration-300',
